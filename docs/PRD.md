...
- 流程：
  1. 用户点击"微信一键登录"按钮
  2. 前端调用微信 JS-SDK 获取临时 code
  3. 前端将 code、昵称、头像等信息提交到后端 `/auth/wx-login`
  4. 后端返回 access_token，前端保存到本地（localStorage/sessionStorage）
  5. 登录成功后自动跳转到主页面
- 交互说明：
  - 登录中显示 loading
  - 登录失败弹出错误提示
  - 登录成功后自动关闭登录弹窗/页面
- 安全说明：
  - 所有后续接口请求需在 header 携带 `Authorization: Bearer <token>`
- 开发/测试环境下，前端可自动使用测试账号（code: test-code），后端收到该 code 时直接返回测试 token，保证产品流程可用。
- 生产环境必须走真实微信授权流程。
